...
pub use internal::{InternalConst, InternalMut, InternalRef};
pub use item::ItemAccess;
pub use leaf::{LeafConst, LeafMut, LeafRef};
...
pub type ValidationBounds<'a, S> = (
    Option<<S as Storage>::ItemRef<'a>>,
    Option<<S as Storage>::ItemRef<'a>>,
);
...
impl<S: Storage, L: LeafRef<S>, I: InternalRef<S>> Reference<S, L, I> {
    ...
    ) -> Result<ValidationBounds<'a, S>, ValidationError>
    where
        S: KeyOrd,
    {
        ...
        for i in 1..self.item_count() {
            let prev = i - 1;
            if S::key_cmp(
                &self.borrow_item(i.into()).unwrap(),
                &self.borrow_item(prev.into()).unwrap(),
            )
            .is_lt()
            {
...
#[cfg(feature = "dot")]
impl<S: Storage, L: LeafRef<S>, I: InternalRef<S>> crate::dot::Display for Reference<S, L, I>
where
...
impl<'a, S: 'a + Storage, L: LeafConst<'a, S>, I: InternalConst<'a, S>> Reference<S, L, I> {
    ...
    #[inline]
    pub fn get<Q: ?Sized>(&self, key: &Q) -> Result<Option<S::ItemRef<'a>>, usize>
    where
        S: KeyPartialOrd<Q>,
    {
        match &self.desc {
...
pub type Mut<'a, S> =
    Reference<S, <S as StorageMut>::LeafMut<'a>, <S as StorageMut>::InternalMut<'a>>;
...
impl<'a, S: 'a + StorageMut, L: LeafMut<'a, S>, I: InternalMut<'a, S>> Reference<S, L, I> {
    ...
    pub fn append(&mut self, separator: S::Item, other: Buffer<S>) -> Offset {
        match (&mut self.desc, other) {
            (Desc::Internal(node), Buffer::Internal(other)) => node.append(separator, other),
            (Desc::Leaf(node), Buffer::Leaf(other)) => node.append(separator, other),
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            ...
            Self::Internal(inner) => inner
                .next()
                .map(|(left, item, right)| (Some(left), item, Some(right))),
        }
    }
}
