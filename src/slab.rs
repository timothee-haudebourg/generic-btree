...
mod map {
    use super::*;
    use crate::{
        btree::{ItemOrd, ItemPartialOrd, KeyOrd, KeyPartialOrd},
        map::Binding,
    };
    ...
    impl<K, V> crate::map::MapStorageMut for MapStorage<K, V> {
        ...
        fn split_mut<'a>(binding: &'a mut Binding<K, V>) -> (&'a K, &'a mut V)
        where
            Self: 'a,
        {
            binding.into()
    ...
    where
        ...
        K: Borrow<Q>,
    {
        fn key_partial_cmp<'r>(binding: &Self::ItemRef<'r>, other: &Q) -> Option<Ordering>
        where
            Self: 'r,
        {
    ...
    where
        K: PartialOrd,
    {
        fn key_partial_cmp<'r>(
            binding: &Self::ItemRef<'r>,
            other: &crate::map::Inserted<K, V>,
        ) -> Option<Ordering>
        where
            Self: 'r,
        {
    ...
    where
        K: Ord,
    {
        fn key_cmp<'r, 's>(binding: &Self::ItemRef<'r>, other: &Self::ItemRef<'s>) -> Ordering
        where
            Self: 'r + 's,
        {
        ...
        V1: PartialOrd<V2>,
    {
        fn item_partial_cmp<'r, 's>(
            binding: &&'r Binding<K1, V1>,
            other: &&'s Binding<K2, V2>,
        ) -> Option<Ordering>
        where
            Self: 'r,
            MapStorage<K2, V2>: 's,
    ...
    where
        ...
        V: Ord,
    {
        fn item_cmp<'r, 's>(binding: &&'r Binding<K, V>, other: &&'s Binding<K, V>) -> Ordering
        where
            Self: 'r + 's,
        {
            binding.key.cmp(&other.key)
